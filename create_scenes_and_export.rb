# Encoding: UTF-8
# ==============================================================================
# ПРОЕКТ 4020-НМ / PECHNIK-ENGINEERING-HUB (v76.5)
# ЧАСТЬ 1: ГРАФИЧЕСКИЙ КОНВЕЙЕР И СВЕРХНАДЁЖНЫЙ ИЕРАРХИЧЕСКИЙ СКАНЕР МОДЕЛИ
# ==============================================================================
require 'fileutils'
require 'sketchup'
  ...
  class SceneExporter
    class << self
      ...
      def ensure_project_folders
        ...
        ["#{base_path}/00_my_scripts", "#{base_path}/01_scenes", "#{base_path}/02_specifications", "#{base_path}/03_web_guide"].each do |folder|
          FileUtils.mkdir_p(folder) unless Dir.exist?(folder)
        end
      end

      # ========================================================================
      # ЗАДАЧА 1: УНИВЕРСАЛЬНЫЙ ГРАФИЧЕСКИЙ КОНВЕЙЕР (БЕЗУПРЕЧНЫЙ МАСШТАБ)
      # ========================================================================
      def export_scenes_to_png(mode = :custom)
        ...
        when :top
          ...
          view.set_direction([0, 0, -1])
        when :iso
          sub_folder = "iso_view"
          view.camera.perspective = true
          view.set_direction([1, -1, -1])
        else
          sub_folder = "custom_view"
        end

        # 🔥 НАДЁЖНЫЙ ФИКСАТОР КАМЕРЫ ДЛЯ ИСКЛЮЧЕНИЯ СДВИГА КАРТИНКИ
        puts "📐 Автоматическое центрирование камеры по границам всей модели..."
        view.zoom_extents
        ...
        puts "🏗 ========================================================"
        puts "🚀 СКОРОСТНОЙ КОНВЕЙЕР: Послойный экспорт кадров под интерфейс"
        puts "============================================================"
        ...
        (1..54).each do |current_row|
          ...
          puts "[#{sprintf('%02d', current_row)}/54] Кадр зафиксирован по эталону слоев."
        end

        UI.messagebox("🎉 [ПАКЕТНЫЙ КОНВЕЙЕР ВЫПОЛНЕН]\n Кадры успешно сохранены в /01_scenes/#{sub_folder}/")
      end

      # ========================================================================
      # ЗАДАЧА 2: ТОТАЛЬНЫЙ СИНТЕЗ ОБЩЕЙ СПЕЦИФИКАЦИИ С ПОРЯДОВЫМ УЧЕТОМ v76.5
      # ========================================================================
      def export_materials_specification
        ...
        # Сквозной рекурсивный сканер с автоматическим подхватом русских названий
        scan_hierarchy = ->(instance, current_row = nil, current_palette = nil, transform = Geom::Transformation.new) {
          ...
          if instance.respond_to?(:definition)
            ...
            if clean_name_down.include?('лицевой') || clean_name =~ /^LF/
              current_palette = :facade
            elsif clean_name_down.include?('полнотелый') || clean_name_down.include?('строительный') || clean_name =~ /^SP/
              current_palette = :building
            elsif clean_name_down.include?('шб-') || clean_name_down.include?('шамот') || clean_name =~ /^SH8/
              current_palette = :refractory
            elsif clean_name_down.include?('дверца') || clean_name_down.include?('чугун') || clean_name_down.include?('колосник') || clean_name_down.include?('литье')
              current_palette = :iron
            end
        ...
        File.open(output_path, "w:UTF-8") do |file|
          file.puts "====================================================================================="
          file.puts "             PRODUCTION SPECIFICATION REPORT (HIERARCHY SYSTEM v76.5)                "
          file.puts "====================================================================================="
          ...
          file.puts "\n====================================================================================="
          file.puts "                     МАТРИЦА ПОРЯДОВОГО РАСХОДА КИРПИЧА                              "
          file.puts "====================================================================================="
          file.puts sprintf(" %-12s | %-15s | %-15s | %-15s | %s", "НОМЕР РЯДА", "ФАСАД (ШТ)", "СТРОИТ (ШТ)", "ШАМОТ (ШТ)", "ЛИТЬЕ")
          file.puts "-------------------------------------------------------------------------------------"
          
          @row_brick_matrix.keys.reject(&:nil?).sort.each do |row|
            lf_cnt = @row_brick_matrix[row]["LF"]
            sp_cnt = @row_brick_matrix[row]["SP"]
            sh8_cnt = @row_brick_matrix[row]["SH8"]
            casting_val = @row_brick_matrix[row]["casting"] ? @row_brick_matrix[row]['casting'] : "Нет"

            lf_str = lf_cnt > 0 ? (lf_cnt % 1 == 0 ? lf_cnt.to_i.to_s : lf_cnt.to_s) : "0"
            sp_str = sp_cnt > 0 ? (sp_cnt % 1 == 0 ? sp_cnt.to_i.to_s : sp_cnt.to_s) : "0"
            sh8_str = sh8_cnt > 0 ? (sh8_cnt % 1 == 0 ? sh8_cnt.to_i.to_s : sh8_cnt.to_s) : "0"

            file.puts sprintf(" Ряд %02d | %-14s | %-14s | %-14s | Литье: %s", row, lf_str, sp_str, sh8_str, casting_val)
          end
          ...
          file.puts "====================================================================================="
          file.puts sprintf(" %-45s : %12s pcs", "LF (Всего кирпича лицевого по палитре)", total_lf_pcs.to_i.to_s)
          file.puts sprintf(" %-45s : %12s pcs", "SP (Всего кирпича строительного по палитре)", total_sp_pcs.to_i.to_s)
          file.puts sprintf(" %-45s : %12s pcs", "SH8 (Всего кирпича огнеупорного по палитре)", total_sh8_pcs.to_i.to_s)
          if @total_finish_table_area > 0.0
            file.puts sprintf(" %-45s : %12s m2", "FINISH-TABLE (Керамогранит столешницы)", sprintf("%.2f", @total_finish_table_area))
          end
          file.puts "-------------------------------------------------------------------------------------"
        ...
        UI.messagebox("🎉 [ИЕРАРХИЧЕСКИЙ СИНТЕЗ v76.5 ВЫПОЛНЕН]\n Смета и порядовая матрица успешно обновлены!")
      end
