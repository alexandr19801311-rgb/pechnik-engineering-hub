# Encoding: UTF-8
# ==============================================================================
# ПРОЕКТ 4020-НМ / v77.5 -- СЖАТЫЙ ВЕБ-ГЕНЕРАТОР PDF/HTML (ПОБЛОЧНАЯ СБОРКА)
# Файл: generate_html_guide.rb — ЧАСТЬ 1: БЭКЕНД И УМНЫЙ ПАРСЕР
# ==============================================================================
...
def parse_specification(file_path)
  ...
  File.foreach(file_path, encoding: 'utf-8:utf-8') do |line|
    ...
    if up_line.include?("МАТРИЦА ПОРЯДОВОГО РАСХОДА")
      ...
      next
    elsif up_line.include?("ИТОГОВЫЙ СВОДНЫЙ РАСХОД")
      current_section = :none
    end
    ...
    if current_section == :matrix && clean_line =~ /^[Рр]яд\s*(\d+)/
      ...
      if parts.length >= 5
        ...
        rows_data[row_num] = {
          'facade' => f_val, 'stroit' => s_val, 'shamot' => sh_val,
          'casting' => c_val, 'note' => DEFAULT_NOTES.fetch(row_num, "Технические примечания отсутствуют.")
        }
      end
    end
  end
...
# ==============================================================================
# ЧАСТЬ 2: АВТОМАТИЧЕСКИЙ СБОРЩИК ИНТЕРФЕЙСА И CSS-ПЕЧАТИ PDF (v77.5)
# ==============================================================================
def generate_html
  puts "[+] Старт сборки интерактивного веб-руководства v77.5..."
  ...
  # Собираем скелет по кусочкам через массив, чтобы обойти любые лимиты чата
  html_lines = []
  ...
  html_lines << "body{background-color:var(--bg-main);color:var(--text-main);display:flex;height:100vh;overflow:hidden;}"
  
  # Спецификация стилей печати книги PDF (Каждый ряд на А4, чистый белый фон)
  html_lines << "@media print{"
  html_lines << "body{background:#fff!important;color:#000!important;height:auto!important;overflow:visible!important;display:block!important;}"
  html_lines << ".sidebar,.top-bar,.total-summary-btn,.mode-switch,.controls{display:none!important;}"
  html_lines << ".main-content{display:block!important;overflow:visible!important;height:auto!important;width:100%!important;}"
  html_lines << ".print-page{page-break-after:always!important;page-break-inside:avoid!important;display:block!important;height:270mm!important;padding-top:5mm!important;border-bottom:1px dashed #ccc!important;margin-bottom:20px!important;}"
  html_lines << ".print-row-title{display:block!important;font-size:20pt!important;color:#000!important;font-weight:bold!important;margin-bottom:10px!important;border-bottom:2px solid #000!important;padding-bottom:3px!important;}"
  html_lines << ".viewer-container{display:flex!important;flex-direction:row!important;height:120mm!important;gap:10px!important;margin-bottom:10px!important;page-break-inside:avoid!important;}"
  html_lines << ".view-pane{border:1px solid #aaa!important;background:#fff!important;flex:1!important;height:100%!important;display:flex!important;flex-direction:column!important;position:relative!important;}"
  html_lines << ".pane-title{position:absolute!important;top:5px!important;left:5px!important;background:#000!important;color:#fff!important;padding:2px 6px!important;font-size:9pt!important;font-weight:bold!important;border-left:3px solid #ff5722!important;}"
  html_lines << ".img-wrapper{flex:1!important;display:flex!important;align-items:center!important;justify-content:center!important;background:#fff!important;padding:5px!important;}"
  html_lines << ".img-wrapper img{filter:brightness(1.05)!important;max-height:110mm!important;max-width:100%!important;object-fit:contain!important;}"
  html_lines << ".note-container{background:#f5f5f5!important;border:1px solid #aaa!important;border-left:5px solid #ff5722!important;color:#000!important;padding:8px 12px!important;margin-bottom:10px!important;height:auto!important;page-break-inside:avoid!important;display:block!important;}"
  html_lines << ".note-text{color:#000!important;font-size:10pt!important;}.note-text span{font-weight:bold!important;color:#ff5722!important;}"
  html_lines << ".info-panel{background:#fff!important;border:1px solid #aaa!important;color:#000!important;height:auto!important;padding:8px 12px!important;display:grid!important;grid-template-columns:repeat(4,1fr)!important;gap:10px!important;page-break-inside:avoid!important;}"
  html_lines << ".stat-block{border-right:1px dashed #aaa!important;padding-right:5px!important;display:flex!important;flex-direction:column!important;}"
  html_lines << ".stat-block:last-child{border-right:none!important;}.stat-label{color:#555!important;font-size:8pt!important;text-transform:uppercase!important;}"
  html_lines << ".stat-value,.stat-value span{color:#000!important;font-size:12pt!important;font-weight:bold!important;}"
  html_lines << "#infoPanel,#rowNoteBlock,.viewer-container{display:none!important;}.print-only-deck{display:block!important;}.modal-overlay{display:none!important;}"
  html_lines << "}"
  # Экранные стили планшета и интерактивный каркас разметки
  html_lines << ".print-only-deck{display:none;}"
  ...
  html_lines << ".nav-btn{padding:6px 12px;background-color:var(--border);border:none;border-radius:4px;color:#fff;cursor:pointer;font-size:0.85rem;}"
  ...
  # Интерактивный HTML-скелет
  html_lines << "<div class='sidebar'><div class='sidebar-header'><h1>🚀 ПРОЕКТ 4020-НМ</h1><p>Полевой веб-интерфейс v77.5 | Александр</p></div>"
  ...
  html_lines << "<div class='main-content'><div class='top-bar'><h2>РЯД --</h2>"
  ...
  # Невидимый на экране контейнер-лента для автогенерации PDF-книги
  ...
  # ==============================================================================
  # ЧАСТЬ 3 (ОБНОВЛЕНИЕ): JAVASCRIPT-ДВИЖОК АВТОГЕНЕРАЦИИ СТРАНИЦ КНИГИ PDF
  # ==============================================================================
  # ==============================================================================
  # ЧАСТЬ 3 (ИСПРАВЛЕНИЕ): СИНТАКСИС ИНИЦИАЛИЗАЦИИ РЯДОВ ДЛЯ БРАУЗЕРА
  # ==============================================================================
  html_lines << "<script>"
  html_lines << "const matrixData = #{rows_json};"
  html_lines << "const sortedRows = Object.keys(matrixData).map(Number).sort((a,b)=>a-b);"
  
  # ИСПРАВЛЕНО: Заменили .at(0) на стандартный JS-индекс [0]
  ...
  html_lines << "    page.innerHTML = '<div class=\"print-row-title\">РЯД ' + fIdx + '</div><div class=\"print-media-container\" style=\"display:flex!important;flex-direction:row!important;height:120mm!important;gap:10px!important;margin-bottom:10px!important;\"><div class=\"view-pane\"><div class=\"pane-title\">ПЛАН</div><div class=\"img-wrapper\"><img src=\"' + topPath + 'row_' + fIdx + '.png\"></div></div><div class=\"view-pane\"><div class=\"pane-title\">ИЗОМЕТРИЯ</div><div class=\"img-wrapper\"><img src=\"' + isoPath + 'row_' + fIdx + '.png\"></div></div></div><div class=\"note-container has-note\"><div class=\"note-text\"><span>ИНФО:</span>' + (d['note'] || 'Примечания отсутствуют.') + '</div></div><div class=\"info-panel\"><div class=\"stat-block\"><div class=\"stat-label\">Облицовка</div><div class=\"stat-value\">' + (d['facade'] || 0) + ' шт.</div></div><div class=\"stat-block\"><div class=\"stat-label\">Забутовка</div><div class=\"stat-value\">' + (d['stroit'] || 0) + ' шт.</div></div><div class=\"stat-block\"><div class=\"stat-label\">Шамот</div><div class=\"stat-value\">' + (d['shamot'] || 0) + ' шт.</div></div><div class=\"stat-block\"><div class=\"stat-label\">Литье</div><div class=\"stat-value\" style=\"font-size:9pt;\">' + (d['casting'] || 'Нет') + '</div></div></div>';"
...
"
  ...
  html_lines << "  const mR = Math.max(...sortedRows); document.getElementById('currentRowTitle').innerText = 'РЯД ' + String(currentRow).padStart(2, '0') + ' / ' + String(mR).padStart(2, '0');"
  ...
  # Моментальная сборка и запись файла без пустых строк
  full_html_page = html_lines.join("\n")
  File.write(OUTPUT_HTML, full_html_page, mode: 'w:utf-8')
  puts "[+] Полевой интерфейс v77.5 с PDF-модулем успешно сгенерирован: #{OUTPUT_HTML}"
end
